...
data_vars::data_vars(QString p_nm, double p_val, double lbnd, double tbnd, QString p_units, QObject* parent):
    m_param_nm(p_nm), m_param_val(p_val), m_l_bound(lbnd), m_t_bound(tbnd), m_param_units(p_units){
    Q_UNUSED(parent);
}
...
void data_vars::setParam_val(const double n_param_val){
    if(n_param_val!=m_param_val){
        qDebug()<<Q_FUNC_INFO<<" n_param_val="<<n_param_val;
        m_param_val=n_param_val;
